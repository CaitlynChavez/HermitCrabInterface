...
write_to_excel = True #True #not set up
write_to_video = True #True #DISABLES VIDEO WRITING
...
BoxInfoList = []
time_date = time.strftime("%d-%m-%Y-%H_%M_%S", time.localtime())
...
def readVideoDirectoryForVideo():
    mypath = 'video_files/'
    ...
    while valid_input == False:
        user_input = input("Please enter number of the video you wish to select\n")
        if user_input in file_dict.keys():
            valid_input = True
            video_name = file_dict[user_input]#.replace(".jpg", "").replace(".png","").replace(".jpeg","").replace("-"," ").replace("_"," ").title()
            print("["+video_name+"]", "selected!")

            filePath = mypath + file_dict[user_input]
            file_name = video_name.replace(".mov","")
...
def ExportMLInformationCSV(csvFileName):
    with open(csvFileName+'.csv', 'w', newline='') as file:
        writer = csv.writer(file)

        box_list_length = len(BoxInfoList)
        if len(BoxInfoList) > 0:
...
def calcDistance(old_coord, new_coord):
    x1 = old_coord[0]
    y1 = old_coord[1]
    x2 = new_coord[0]
    y2 = new_coord[1]
...
class BoxInfo(object):
    ...
    def updateCoord(self, coord):
        ...
        if self.old_coord != (-1,-1):
            ...
            new_distance = calcDistance(self.old_coord, new_coord)
    ...
    def checkLaps(self, coord):
        lap_marker_distance = calcDistance(coord, self.start_coords)
...
video_output = cv2.VideoWriter('tracking_videos/' + video_name+time_date+".avi", cv2.VideoWriter_fourcc('M','J','P','G'), 40, frame_size)
...
while True:
    ...
    if find_cup_size == True:
        ...
        cup_average = (cup_width + cup_height)/2
        cup_ratio = (cup_size/cup_average)
        print("cup width", cup_width, "cup height", cup_height, "cup average", cup_average, "cup ratio to cm", cup_ratio)
    ...
    for box in boxes:
        ...
        BoxInfoList[iterator].updateCoord(new_coord) #updates coordinate of each labeled box
        BoxInfoList[iterator].checkLaps(new_coord)
        iterator +=1
    ...
    while number_of_crabs_to_track > 0:
        if key == ord("q"):
        ...
        if label_name == "quit" or label_name == "q":
            number_of_crabs_to_track = 0
            break
        BoxInfoList.append(BoxInfo(label_name)) #adds info for new box
        number_of_crabs_to_track = number_of_crabs_to_track -1
    ...
    if print_counter == print_threshold:
        ...
        for i in range(0, len(BoxInfoList)):
            BoxInfoList[i].printBoxInfo()


    #creates a distance threshold for moving vs waiting on first run
    if distance_threshold == 0:
      ...
      if len(temp_distance_list) != 0:
        ...
        distance_threshold = average*0.06 #was 0.06
    ...
    if key == ord("s"):
        ...
        trackers.add(tracker, frame, box)
        label_name  = input("Please enter name of label:\n")
        BoxInfoList.append(BoxInfo(label_name)) #adds info for new label box
...
if write_to_video == True:
  ...
  print("Labelled video recorded to",video_path)
# ExportExcel(video_name)
# ExportMinuteInformationExcel(video_name)
ExportMLInformationCSV(video_name)
